...
check("banner shows version and workspace", t.saw("koda 0.1.0"), t)
check("banner shows the workspace path", t.saw(os.path.basename(ws)), t)
...
check("auto-approve flagged in the composer", t.saw("auto"), t)
...
t2.read(8.0, until="approve edit_file")
t2.read(0.6)
check("modal titled with the tool name", t2.saw("approve edit_file"), t2)
...
check("modal shows key hints", t2.saw("y once") and t2.saw("n deny"), t2)
t2.send("y")
...
t6.read(1.0)
check("plan block has a heading", t6.saw("plan"), t6)
check("plan card shows a progress gauge", t6.saw("3/3") and t6.saw("████"), t6)
...
check("welcome card has a heading", t8.saw("koda 0.1.0"), t8)
check("welcome offers examples", t8.saw("what does this project do?"), t8)
t8.send("explain @vie")
...
t8.send("/help\r")
t8.read(3.0, until="input history")
check("help is two blocks, two columns",
      t8.saw("Commands") and t8.saw("Keys") and t8.saw("input history"), t8)
t8.close()
...
t3.send("/")
t3.read(2.0, until="/compact")
check("completion hint lists commands", t3.saw("/compact") and t3.saw("/models"), t3)
t3.send("\x15")  # ctrl+u: clear the slash so the hint stops overlaying
t3.send("/help\r")
t3.read(3.0, until="expand last tool output")
check("/help documents keys", t3.saw("expand last tool output"), t3)
check("/help fits on one screen", t3.screen().count("Commands") == 1, t3)
t3.send("/tools\r")
t3.read(1.5, until="run_command")
check("/tools lists the tool suite", t3.saw("edit_file") and t3.saw("find_files"), t3)
t3.send("/auto\r")
t3.read(1.0)
check("/auto toggles approval mode", t3.saw("auto-approve on"), t3)
t3.send("/think\r")
t3.read(1.0)
